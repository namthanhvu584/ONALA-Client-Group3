...
function OrderInfo1() {
    ...
    const { user_id, selectedItemsList } = location.state || {};
    
    

    const isValidOrder = user_id && selectedItemsList?.length > 0;
    ...
    const onFinish = async (values) => {
        ...
        if (updateSuccess) {
            try {
                const orderData = {
                    user_id,
                    cart_item: selectedItemsList.map(item => ({
    ...
    return (
        <div className="orderInfo">
            ...
            <div id='orderSummary' className='containerGeneral'>
                ...
                {selectedItemsList.map((item, index) => (
                    <div key={index}>
                        ...
                        <div className='generalCost'>
                            <p>TỔNG ĐƠN HÀNG :</p>
                            <FormatCost value={item.price * item.quantity} />
                        </div>
                    </div>
                ))}
                <div className='generalCost'>
                ...
                <div className='generalCost'>
                    <p>THÀNH TIỀN :</p>
                    <FormatCost value={selectedItemsList.reduce((total, item) => total + item.price * item.quantity, 15000)} />
                </div>
            </div>
        </div>
    );
